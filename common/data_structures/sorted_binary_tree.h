...
template <typename T> class SortedBinaryTree : public BinaryTree<T> {
public:
...
template <typename T> bool SortedBinaryTree<T>::search(const T &data) const {
...
template <typename T> void SortedBinaryTree<T>::insert(const T &data) {
  // Binary Tree is instantiated with data, so we don't need to check for
  // nullptr.
...
void SortedBinaryTree<T>::setLeft(std::unique_ptr<BinaryTree<T>> left) {
  SortedBinaryTree<T> *sortedLeft =
      left ? dynamic_cast<SortedBinaryTree<T> *>(left.get()) : nullptr;
...
void SortedBinaryTree<T>::setRight(std::unique_ptr<BinaryTree<T>> right) {
  SortedBinaryTree<T> *sortedRight =
      right ? dynamic_cast<SortedBinaryTree<T> *>(right.get()) : nullptr;
