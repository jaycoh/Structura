...
public:
  Quadtree(Point topLeft, Point bottomRight)
    ...
    if (topLeft_.getX() >= bottomRight_.getX() ||
        topLeft_.getY() >= bottomRight_.getY()) {
...
private:
  ...
  bool isInBoundary(const Point &point) const {
    return point.getX() >= topLeft_.getX() &&
           point.getX() <= bottomRight_.getX() &&
           point.getY() >= topLeft_.getY() &&
           point.getY() <= bottomRight_.getY();
  }
  ...
  void insertImpl(const Point &point, const T &data) {
    ...
    if (point.getX() <= midX) {
      if (point.getY() <= midY) {
        if (!topLeftTree_) {
          topLeftTree_ =
              std::make_unique<Quadtree>(topLeft_, Point(midX, midY));
        }
      ...
      if (point.getY() <= midY) {
        ...
        if (!bottomRightTree_) {
          bottomRightTree_ =
              std::make_unique<Quadtree>(Point(midX, midY), bottomRight_);
        }
  ...
  const T *searchImpl(const Point &point) const {
    if (!topLeftTree_ && hasPoint_ && point_.getX() == point.getX() &&
        point_.getY() == point.getY()) {
      return data_.get();
    }
